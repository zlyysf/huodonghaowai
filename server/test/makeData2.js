...
//var host = 'ysfCentSrvLocal';//'localhost';
var host = 'ec2-23-21-136-120.compute-1.amazonaws.com';
...
function busMakeUsersPhotos1(params,cbFun){
    ...
    var userPhotosAry = [
         { user:{  emailAccount:'email1@abc.com', password:gPassword, name:'user1',height:171,gender:'female',
                   deviceType:gDeviceTypeIphone,
                   latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1  },
           ...
           dates:[{latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1,
                   dateDate:dateDate30,whoPay:0,money:100,monetaryunit:"$",title:"title",description:"description"}
                  ]

         },
         { user:{  emailAccount:'email2@abc.com', password:gPassword, name:'user2',height:172,gender:'female',
                   deviceType:gDeviceTypeIphone,
                   latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1  },
...
function busMakeUsersPhotos2Sequence(params,cbFun){
    ...
    var userPhotosAry = [
         { user:{  emailAccount:'lianyu.zhang+Am1@yasofon.com', password:gPassword, name:'Abe',height:176,gender:'male',
                   deviceType:gDeviceTypeIphone,
                   latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1},
         ...
         { user:{  emailAccount:'lianyu.zhang+Am2@yasofon.com', password:gPassword, name:'Bruce',height:181,gender:'male',
           deviceType:gDeviceTypeIphone,
           latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1  },
         ...
         { user:{  emailAccount:'lianyu.zhang+Am3@yasofon.com', password:gPassword, name:'Carl',height:177,gender:'male',
           deviceType:gDeviceTypeIphone,
           latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1 },
         ...
         { user:{  emailAccount:'lianyu.zhang+Am4@yasofon.com', password:gPassword, name:'David',height:180,gender:'male',
           deviceType:gDeviceTypeIphone,
           latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1},
         ...
         { user:{  emailAccount:'lianyu.zhang+Af1@yasofon.com', password:gPassword, name:'Alice',height:167,gender:'female',
           deviceType:gDeviceTypeIphone,
           latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1 },
         ...
         { user:{  emailAccount:'lianyu.zhang+Af2@yasofon.com', password:gPassword, name:'Betty',height:172,gender:'female',
           deviceType:gDeviceTypeIphone,
           latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1 },
         ...
         { user:{  emailAccount:'lianyu.zhang+Af3@yasofon.com', password:gPassword, name:'Cathy',height:170,gender:'female',
           deviceType:gDeviceTypeIphone,
           latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1 },
         ...
         { user:{  emailAccount:'lianyu.zhang+Af4@yasofon.com', password:gPassword, name:'Daisy',height:164,gender:'female',
           deviceType:gDeviceTypeIphone,
           latlng:gLatlng1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1 },
    ...
    addWidthHeightToUserPhotos2(userPhotosAry,function(){
      ...
      userPhotosAry.forEach(function (userPhotosItem, indexU, array) {
        if (userPhotosItem.user && userPhotosItem.user.name){
...
* @param next - is function(next)
*/
function doMakeUsersDates1SequenceLocalBothSides(params,next){
  handy.log('blue', "running doMakeUsersDates1SequenceLocalBothSides");
  if (!params) params = {};
  params.host = 'localhost';
  if (!params.port) params.port = port;
  if (!params.securePort) params.securePort = securePort;
  ...
  testlib.provideServerLifeCycle(
    {port:params.port,securePort:params.securePort, needInitStore:true, needClearSolr:true, NeedSetConfigDefault:false, notKeepC2dmAuth:true, c2dmAuth:gC2dmAuth},
    function(cbNext){
      doMakeUsersDates1SequenceClientSide(params,function(outData){
        var waitMs = config.config.solrCommitWithin*2+config.config.solrPeriodicThreadInterval*2;
        setTimeout(function(){
          if (cbNext) cbNext();
        },waitMs);//setTimeout
...
}//doMakeUsersDates1SequenceLocalBothSides

/**
 * as there is no socket.io, no complicate logic
 * @param params - contains host,port; ..
 *     @see busMakeUsersDates1Sequence
 * @param cbFun - is function(outData)
 *   outData contains ..
 */
function doMakeUsersDates1SequenceClientSide(params,cbFun){
  handy.log('blue', "doMakeUsersDates1SequenceClientSide enter");
  assert.ok(params.host);
  assert.ok(params.port);
  assert.ok(params.securePort);
  busMakeUsersDates1Sequence(params,function(outDataBus){
    if (cbFun) cbFun(outDataBus);
  });//busMakeUsersDates1Sequence
}//doMakeUsersDates1SequenceClientSide
 ...
 *
 */
function busMakeUsersDates1Sequence(params,cbFun){
    handy.log('blue', "busMakeUsersDates1Sequence enter");
    assert.ok(params.host);
    assert.ok(params.port);
    assert.ok(params.securePort);
    var host = params.host;
    ...
    var userPhotosAry = [
         { user:{  emailAccount:'lianyu.zhang+Am1@yasofon.com', password:gPassword, name:'Abe',height:176,gender:'male',
                   deviceType:gDeviceTypeIphone,
                   latlng:latlngNS0,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1},
           photos:[{image:"/mnt/hgfs/zly/t/tpic/draw.png",notUploadReally:!uploadReally,userId:'no need set',width:'no need set',height:'no need set'}],
           dates:[{latlng:latlngNS0,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1,
                   dateDate:dateDate30,whoPay:0,money:100,monetaryunit:"$",title:"title",description:"description"}]
         },
         { user:{  emailAccount:'lianyu.zhang+Am2@yasofon.com', password:gPassword, name:'Bruce',height:181,gender:'male',
                   deviceType:gDeviceTypeIphone,
                   latlng:latlngNS1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1  },
           photos:[{image:"/mnt/hgfs/zly/t/tpic/draw.png",notUploadReally:!uploadReally,userId:'no need set',width:'no need set',height:'no need set'}],
           dates:[{latlng:latlngNS1,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1,
                   dateDate:dateDate30,whoPay:0,money:100,monetaryunit:"$",title:"title",description:"description"}]
         },
         { user:{  emailAccount:'lianyu.zhang+Am3@yasofon.com', password:gPassword, name:'Carl',height:177,gender:'male',
                   deviceType:gDeviceTypeIphone,
                   latlng:latlngNS2,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1 },
           photos:[{image:"/mnt/hgfs/zly/t/tpic/draw.png",notUploadReally:!uploadReally,userId:'no need set',width:'no need set',height:'no need set'}],
           dates:[{latlng:latlngNS2,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1,
                   dateDate:dateDate30,whoPay:0,money:100,monetaryunit:"$",title:"title",description:"description"}]
         },
         { user:{  emailAccount:'lianyu.zhang+Am4@yasofon.com', password:gPassword, name:'David',height:180,gender:'male',
                   deviceType:gDeviceTypeIphone,
                   latlng:latlngNS3,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1},
           photos:[{image:"/mnt/hgfs/zly/t/tpic/draw.png",notUploadReally:!uploadReally,userId:'no need set',width:'no need set',height:'no need set'}],
           dates:[{latlng:latlngNS3,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1,
                   dateDate:dateDate30,whoPay:0,money:100,monetaryunit:"$",title:"title",description:"description"}]
         },
         { user:{  emailAccount:'lianyu.zhang+Am5@yasofon.com', password:gPassword, name:'Edward',height:180,gender:'male',
                   deviceType:gDeviceTypeIphone,
                   latlng:latlngNS4,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1},
           photos:[{image:"/mnt/hgfs/zly/t/tpic/draw.png",notUploadReally:!uploadReally,userId:'no need set',width:'no need set',height:'no need set'}],
           dates:[{latlng:latlngNS4,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1,
                   dateDate:dateDate30,whoPay:0,money:100,monetaryunit:"$",title:"title",description:"description"}]
         },
         ...
         { user:{  emailAccount:'lianyu.zhang+Af5@yasofon.com', password:gPassword, name:'Eileen',height:164,gender:'female',
                   deviceType:gDeviceTypeIphone,
                   latlng:latlngNS4,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1 },
           photos:[{image:"/mnt/hgfs/zly/t/tpic/draw.png",notUploadReally:!uploadReally,userId:'no need set',width:'no need set',height:'no need set'}],
           dates:[{latlng:latlngNS4,geolibType:gGeolibTypeGoogle,region:gRegionByGoogle1,
...
//doMakeUsersDates1SequenceLocalBothSides({uploadReally:false,disableNotification:true},null);
//doMakeUsersDates1SequenceLocalBothSides({uploadReally:true,disableNotification:true},null);
doMakeUsersDates1SequenceClientSide({host:host,port:port,securePort:securePort, uploadReally:true,disableNotification:true},null);
