...
public class Renren implements Parcelable {
    ...
    private static final String LOG_TAG = "Renren-SDK";
    ...
    public Renren(Parcel in) {
        ...
        Log.d(LOG_TAG, "Renren(Parcel in), bundle=" + bundle.toString());
     ...
     *            授权状态监听器
     */
    public void authorize(Activity activity, PasswordFlowRequestParam param,
            final RenrenAuthListener listener) {
        if (this.isSessionKeyValid()) {
            listener.onComplete(new Bundle());
            return;
     ...
     */
    public PasswordFlowResponseBean authorize(PasswordFlowRequestParam param)
     ...
     * @param status
     *            要发送的状态
     * @return 若成功，返回发送的 {@link StatusSetResponseBean}对象，否则返回null
     * @thorws RenrenException
     * @throws Throwable
     */
    public StatusSetResponseBean publishStatus(StatusSetRequestParam status)
            throws RenrenException, Throwable {
     ...
     * 
     *            <p>
     *            注意：需要在AndroidManifest.xml中添加Activity描述行
     *            <p>
     ...
     *            <p>
     *            同时需要在AndroidManifest.xml中添加Activity行
     *            <p>
     *            &ltactivity android:name=
    ...
    /**
     * 发布状态<br>
     * 使用sdk提供的Widget窗口发状态 参见http://wiki.dev.renren.com/wiki/Status_dialog
     * 
     * @param activity
     *            当前activity
     * @param listener
    ...
    /**
     * 发布新鲜事<br>
     * 同步调用feed.publishFeed接口 参见http://wiki.dev.renren.com/wiki/Feed.publishFeed
     * 
     * @param feed
     *            要发送的新鲜事
     * @return 若发送成功，返回已发送的新鲜事，否则返回null
     * @throws RenrenException
     * @throws Throwable
     */
    public FeedPublishResponseBean publishFeed(FeedPublishRequestParam feed)
            throws RenrenException, Throwable {
    ...
    /**
     * 使用Widget窗口发新鲜事 参见http://wiki.dev.renren.com/wiki/Feed_dialog
     * 
     * @param activity
     *            显示此widget的activity
     * @param listener
    ...
    /**
     * 以startActivity方式根据用户传入的 Intent 对象启动相应的AbstractRenrenRequestActivity对象
     * 
     * @param activity
     *            当前Activity
     * @param intent
    ...
    /**
     * 以startActivityForResult方式根据用户传入的 Intent
     * 对象启动相应的AbstractRenrenRequestActivity对象
     * 
     * @param activity
     *            当前Activity
     * @param intent
     ...
     * 
     *            <p>
     *            注意：调用此方法需在AndroidManifest.xml中注册以下代码： &ltactivity
     *            android:name=
     *            "com.renren.api.connect.android.AuthorizationHelper$BlockActivity"
     *            android:theme="@android:style/Theme.Dialog" &gt&lt/activity&gt
     *            &ltactivity android:name=
     ...
     * 
     *            <p>
     *            详情请参考 http://wiki.dev.renren.com/wiki/Photos.createAlbum
     */
     ...
     * 
     *            <p>
     *            详情请参考 http://wiki.dev.renren.com/wiki/Photos.upload
     */
     ...
     *             详情请参考 http://wiki.dev.renren.com/wiki/Photos.upload
     */
    public PhotoUploadResponseBean publishPhoto(
    ...
    public FriendsGetFriendsResponseBean getFriends(
            FriendsGetFriendsRequestParam param) throws RenrenException,
            Throwable {
        FriendsHelper helper = new FriendsHelper(this);
        return helper.getFriends(param);
    }
     ...
     * 
     *             <p>
     *             详情请参考 http://wiki.dev.renren.com/wiki/Photos.getAlbums
     */
    ...
    private String request(Bundle parameters, String format) {
        ...
        if (isSessionKeyValid()) {
            parameters.putString("session_key",
                    accessTokenManager.getSessionKey());
        }
    ...
    public long getExpireTime() {
        return accessTokenManager.getExpireTime();
    }
     ...
     * @return
     */
    public IRenrenPay getRenrenPay(Context context,
            IPayRepairListener repairListener) {
