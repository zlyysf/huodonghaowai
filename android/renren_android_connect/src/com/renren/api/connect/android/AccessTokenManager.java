...
public class AccessTokenManager implements Parcelable {
    ...
    public AccessTokenManager(Parcel in) {
        ...
        Log.d(LOG_TAG,
                "AccessTokenManager(Parcel in), bundle=" + bundle.toString());
        if (bundle != null) {
            ...
            Log.d(LOG_TAG, "AccessTokenManager(Parcel in), uid=" + uid);
        }
        compareWithConfig();
    }
     ...
     * 返回当前session key是否有效
     * 
     * @return true - session key有效 false - session key无效
     */
            ...
            public void onSharedPreferenceChanged(
                ...
                if ((sessionKey != null && !sessionKey.equals(sKey))
                        || (sessionSecret != null && !sessionKey.equals(secret)
                                || (sessionKey == null && sKey != null) || (sessionSecret == null && secret != null))
                        || (uid != userId)) {
                    initSessionKey();
                }
            }
     ...
     * @param sessionSecret
     * @param expireTime
     *            Session key 的过期时间，单位为秒
     * @param uid
     */
    private void storeSessionParams(String sessionKey, String sessionSecret,
            long sessionCreateTime, long expires, long uid) {
        if (sessionKey == null || sessionSecret == null) {
            this.clearPersistSession();
            return;
        }
        ...
        editor.putLong(RENREN_SDK_CONFIG_PROP_SESSION_CREATE_TIME,
                sessionCreateTime);
