...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
		...
		self.textLabel.textColor = [UIColor colorWithRed:87.0/255.0 green:108.0/255.0 blue:137.0/255.0 alpha:1.0];
		self.textLabel.backgroundColor = [UIColor clearColor];
		self.textLabel.textAlignment = UITextAlignmentCenter;        
        self.textLabel.text = NSLocalizedString(@"上拉载入更多", @"");
...
- (void) setShowActivityIndicator:(BOOL)showActivityIndicator {
    ...
    if (showActivityIndicator && !self.activityView) {
        ...
        self.activityView.center = CGPointMake(self.frame.origin.x+80, self.frame.size.height / 2);
        [self addSubview:self.activityView];
        [self.activityView startAnimating];
        self.textLabel.text = NSLocalizedString(@"载入中...", @"");
    }
